...
function getProfileData(req, userId, hideControls) {
  ...
  const certs = req.db
    .all('SELECT cert, opo_approved FROM user_certs WHERE user = ?', userId)
...
export function getDriverCertRequest(req, res) {
  ...
  const driver_certs = req.db.all('SELECT cert, opo_approved FROM user_certs WHERE user = ?', userId)
...
export function postDriverCertRequest(req, res) {
  ...
  req.db.run(`DELETE FROM user_certs WHERE user = ? ${!res.locals.is_opo ? 'and opo_approved = 0' : ''}`, userId)
  ...
  certs
    ...
    .map(cert => req.db.run(
      'INSERT OR IGNORE INTO user_certs (user, cert, opo_approved) VALUES (?, ?, ?)',
